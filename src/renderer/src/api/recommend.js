...
export async function getRecentLike() {
  const data = await request({
    url: `/recent/listen/list?timestamp=${Date.now()}`,
    method: 'post'
  })
  return data
}
export async function getRadio() {
  let radio = [3136952023, 5320167908, 5300458264, 5362359247, 5327906368, 5341776086]
  radio = radio.map((id) =>
    request({
  ...
  radio = await Promise.all(radio)
  return radio.map(i=>i?.playlist)
}
...
export async function getHomepageBlock(blockCodeOrderList, offset = 0, refresh = true) {
  const cursor = {
    blockCodeOrderList: Array.isArray(blockCodeOrderList)
      ? blockCodeOrderList
      : [blockCodeOrderList]
  }
  return request({
