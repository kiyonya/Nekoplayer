  ...
  <div class="page">
    ...
    <div class="head">
      ...
      <div class="recent">
        <h3>最近常听</h3>
        <div class="songs">
          <Song_Small v-for="songInfo in recent?.slice(0, (deviceScreenSize < 1 ? 3 : 6))" :name="songInfo?.song?.name"
            :cover="songInfo?.song?.al?.picUrl" :artist="songInfo?.song?.ar" :id="songInfo?.song?.id"
    ...
    <div class="selector" ref="totop">
      <select name="" id="" class="select" @click="
        ($event) => {
    ...
    <div class="viewer">
      <div class="playlist_all grid g-shell-6" v-if="view === 'playlist_all'">
        <PlaylistCard v-for="pl in [...playlistCreate?.slice(1), ...playlistCollect]" :name="pl.name"
      ...
      <div class="playlist_collect grid g-shell-6" v-if="view === 'playlist_collect'">
        <PlaylistCard v-for="pl in playlistCollect" :name="pl.name" :cover="pl.coverImgUrl" :id="pl.id"
          @playall="player.playPlaylist(null, null, { type: 'playlist', id: pl.id }, true)">
        </PlaylistCard>
      </div>
      <div class="playlist_create grid g-shell-6" v-if="view === 'playlist_create'">
        <PlaylistCard v-for="pl in playlistCreate?.slice(1)" :name="pl.name" :cover="pl.coverImgUrl" :id="pl.id"
          @playall="player.playPlaylist(null, null, { type: 'playlist', id: pl.id }, true)">
        </PlaylistCard>
      </div>
      <div class="album_all grid g-shell-6" v-if="view === 'album_all'">
        <AlbumCard v-for="al in albums?.data" :name="al.name" :size="al.size" :cover="al.picUrl" :id="al.id"
          :hideInfo="true" @playall="player.playPlaylist(null, null, { type: 'playlist', id: al.id }, true)">
        </AlbumCard>
      </div>
      <div class="mv_all grid g-shell-5" v-if="view === 'mv_all'">
        <MvCard v-for="mv in mvs?.data" :cover="mv.coverUrl" :name="mv.title" :duration="mv.durationms" :id="mv.vid">
        </MvCard>
      </div>
      <div class="recent-play-list" v-if="view === 'recent_all'">
        <Song v-for="(song, index) in recentAll" :trackDetial="{
          name: song.data.name,
          ...
          alia: song.data.alia,
          mv: song.data.mv
        }" :index="index" :source="{ type: 'recent', id: '' }" @play="playRecent">
        </Song>
      </div>
    </div>
  </div>
...
async function load(uid) {
  let tasks = [
    ...
    async () => {
      getUserCollectedAlbums().then(data => { albums.value = data; data = null })
    },
    async () => {
      getUserCollectedMV().then(data => { mvs.value = data; data = null })
    },
    async () => {
...
onUnmounted(() => {
  playlistCreate.value = null
  playlistCollect.value = null
